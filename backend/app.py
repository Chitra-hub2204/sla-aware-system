...
def create_app():
    ...
    # Initialize DB + optional metrics thread
    with app.app_context():
        ...
        try:
            ...
            print("⚠ psutil not available — process metrics disabled")
    ...
    # ============================================================================
    # Mixed-mode SLA (60% OK / 40% breach)
    # ============================================================================
    def check_sla_breach(latency: float, uptime: float) -> bool:
        if latency > 700 or uptime < 95:
            return True  # Hard breach
        return random.random() < 0.40  # 40% chance
    ...
    def update_order_sla_status(order_id: int, latency: float, uptime: float):
        ...
        breached = check_sla_breach(latency, uptime)
        ...
        if breached:
            ...
            if previous_status != "BREACHED":
                email_service.send_breach_alert(
                    service_id=f"order-{order_id}", latency=latency, uptime=uptime
                )
                db.session.add(
                    Alert(
                        order_id=order_id,
    ...
    def create_order():
        ...
        try:
            ...
            # NETCONF mock activation
            try:
                orchestrator.order_service(
            ...
            # Metrics
            latency = float(order.sla_latency_ms)
            uptime = float(order.sla_uptime_pct)

            sla_latency.labels(service_id=service_id).set(latency)
            sla_latency_histogram.labels(service_id=service_id).observe(latency)
            sla_uptime.labels(service_id=service_id).set(uptime)
            service_activation_status.labels(service_id=service_id).set(1)

            # SLA evaluation
            update_order_sla_status(order.id, latency, uptime)
    ...
    def get_order(order_id):
        ...
        metrics = MetricRecord.query.filter_by(order_id=order.id).order_by(
            MetricRecord.timestamp.asc()
        )
        alerts = Alert.query.filter_by(order_id=order.id).order_by(
    ...
    def metrics():
        ...
        custom = generate_latest(registry)
        default = generate_latest(DEFAULT_REGISTRY)
        return Response(custom + b"\n" + default, mimetype=CONTENT_TYPE_LATEST)
